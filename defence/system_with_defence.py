...
app.config['SESSION_COOKIE_SECURE'] = False          # Set to True when using HTTPS to ensure cookies are sent over secure channels
app.config['SESSION_COOKIE_SAMESITE'] = 'Strict'     # Protect against Cross-Site Request Forgery (CSRF) attacks
...
def index():
    ...
    if 'user' in session:
        ...
        conn.close()
    return render_template('index_new.html', files=files, user=session.get('user'))
...
# Limit /login to a maximum of 5 attempts per minute per IP.
...
def login():
    ...
    if request.method == 'POST':
        ...
        conn = get_db_connection()
        cursor = conn.execute("SELECT * FROM users WHERE (username = ? OR email = ?) AND password = ?",
...
def upload():
    ...
    conn = get_db_connection()
    conn.execute("INSERT INTO files (filename, original_filename, owner) VALUES (?, ?, ?)",
                 (secure_name, original_filename, session['user']))
    conn.commit()
    conn.close()
